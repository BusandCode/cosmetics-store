...
export default function ProductDetail({ product }: { product: Product }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8">
      <Link href="/" className="text-xs uppercase tracking-widest text-neutral-500 hover:underline">← Back</Link>
      ...
      <div className="grid sm:grid-cols-2 gap-10 mt-6 mb-16">
        <div>
          ...
          {images.length > 1 && (
            <div className="grid grid-cols-5 gap-2">
              {images.map((img, i) => (
                <button key={i} onClick={() => setActiveImg(i)} className={`relative aspect-square bg-neutral-100 ${i === activeImg ? "ring-2 ring-black" : ""}`}>
        ...
        <div>
            ...
            <tbody>
              {product.variants.map((v) => (
                <tr key={v.id} className="border-b border-neutral-200">
                  <td className="py-2">
                    <button onClick={() => setVariantId(v.id)} className={`text-left ${v.id === variantId ? "font-semibold" : "text-neutral-500"}`} disabled={v.stock === 0}>
          ...
          <button
            ...
            onClick={() => variant && addItem({ variantId: variant.id, productName: product.name, productSlug: product.slug, variantLabel: variant.label, priceKobo: variant.priceKobo, image: images[0], maxStock: variant.stock })}
      ...
      <section className="border-t border-black pt-10">
        <h2 className="text-xl font-bold tracking-tight mb-6">Reviews</h2>

        <div className="grid sm:grid-cols-2 gap-10">
          <div className="space-y-6">
            {product.reviews.length === 0 && <p className="text-sm text-neutral-400">No reviews yet — be the first.</p>}
          ...
          <form onSubmit={submitReview} className="space-y-4">
            ...
            <div className="flex gap-1">
              {[1, 2, 3, 4, 5].map((i) => (
                <button key={i} type="button" onClick={() => setRating(i)} className={`w-6 h-6 ${i <= rating ? "bg-black" : "border border-neutral-300"}`} aria-label={`${i} star`} />
              ))}
            </div>
            <textarea value={comment} onChange={(e) => setComment(e.target.value)} rows={4} placeholder="What did you think?" className="w-full border border-black px-3 py-2 text-sm" />
            <p className="text-xs text-neutral-400">Only visible if you've purchased this product — otherwise submitting will show an error.</p>
            {reviewError && <p className="text-sm text-red-700">{reviewError}</p>}
            <button type="submit" disabled={submitting} className="text-xs uppercase tracking-wide bg-black text-white px-4 py-2 hover:bg-neutral-800 transition-colors disabled:opacity-50">
